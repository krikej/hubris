...
impl NotificationHandler for ButtonServer {
    ...
    fn handle_notification(&mut self, bits: u32) {
        ...
        if (bits & notifications::BUTTON_IRQ_MASK) != 0 {
            if let Ok(detected) = self.gpio.detected_falling(BUTTON_PINT_MASK) {
                self.gpio.clear_falling(BUTTON_PINT_MASK);
                self.gpio.clear_status(BUTTON_PINT_MASK);
                if detected != 0 {
...
fn main() -> ! {
    ...
    server.gpio.enable_falling(BUTTON_PINT_MASK);
    sys_irq_control(notifications::BUTTON_IRQ_MASK, true);
